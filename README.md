...
Every ~10 minutes during the US session it ingests the full QQQ options chain from Cboe's delayed public feed and a live NQ futures quote, runs a gamma exposure model over ~12,000 contracts, and produces:
...
| **γ-Ladder** | The runner-up strikes by absolute net gamma — the secondary shelves of the map |
...
- **Session Drift** — how flip, walls, spot, the EM band, and net gamma have moved through the session (08:30–16:00 ET), with a per-snapshot regime strip. Outside market hours the most recent session stays on display
...
Pine Script is sandboxed (no network or file access), so GEXYGEN uses the same delivery mechanism the commercial vendors use on TradingView: a **paste string**. The collector writes a one-line levels string (`FLIP=…;CW=…;PW=…`) to `tv_levels.txt` and displays it on the dashboard with a copy button; the included **GEX Levels** indicator (`gexygen_levels.pine`) parses it and draws the levels with zone shading, configurable styling, and per-level colors.

A NinjaTrader 8 indicator (fully automatic — NinjaScript can read `levels.txt` directly from disk) is planned.
...
CNBC futures quote (@ND.1)  ────────┘         │              gamma curve, session history)
                                              ...
                                              ├──►  levels.txt       (NinjaTrader reader format)
...
- **Polite by design** — the chain is cached with a TTL and never hammered; data requests match the source's actual refresh rate
...
py collector.py --loop 10        # snapshot every 10 minutes
start_collector.cmd              # full session: waits for 08:30 ET, runs to the close, logs
launch_gexygen.cmd               # one click: opens the dashboard + starts the session collector
```

The session window is gated in **ET via zoneinfo** (premarket from 08:30, when fresh overnight open interest has landed, through the 16:00 close marks), so DST never needs schedule changes. A Windows Task Scheduler entry pointed at `start_collector.cmd` makes the whole thing hands-off; a single-instance guard makes double-launches safe.
...
- NinjaTrader 8 indicator reading `levels.txt`
